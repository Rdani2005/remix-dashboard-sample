...
export const Alert = ({
  ...
  return (
    <div
      className={`alert ${dismisable ? "alert-dismissible" : ""} bg-light-${color} d-flex flex-column flex-sm-row p-5 mb-10`}
    >
      <span
      ...
      {dismisable && (
        <button
          ...
          onClick={onExit}
        >
          <span className={`svg-icon svg-icon-1 svg-icon-${color}`}>...</span>
        </button>
      )}
    </div>
  );
